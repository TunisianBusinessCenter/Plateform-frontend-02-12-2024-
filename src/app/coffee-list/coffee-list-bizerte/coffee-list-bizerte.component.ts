...
@Component({
  selector: 'app-coffee-list-bizerte',
  templateUrl: './coffee-list-bizerte.component.html',
  styleUrls: ['./coffee-list-bizerte.component.css']
})
export class CoffeeListBizerteComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Servicemateriaux.getMateriauxEquipement().subscribe((data: any) => {
      ...
      this.AgenciesCoffee = data.filter((item: any) => item.phone_number_commercial === "yes" && item.region === "bizerte");

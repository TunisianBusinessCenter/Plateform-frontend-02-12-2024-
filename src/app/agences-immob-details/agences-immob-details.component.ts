...
@Component({
  selector: 'app-agences-immob-details',
  templateUrl: './agences-immob-details.component.html',
  styleUrls: ['./agences-immob-details.component.css'],
  styles: [`
  .star {
    font-size: 1.5rem;
    color: #d7d730;
...
export class AgencesImmobDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.agenceImmobService.getBiensById(this.idBiens).subscribe(data => {
      ...
      this.Biens = data;
            //Video_url
      this.urlSafe= this.sanitizer.bypassSecurityTrustResourceUrl(this.Biens.videoUrlBien
        );

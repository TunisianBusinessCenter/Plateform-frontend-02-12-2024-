...
@Component({
  selector: 'app-coffee-list-monastir',
  templateUrl: './coffee-list-monastir.component.html',
  styleUrls: ['./coffee-list-monastir.component.css']
})
export class CoffeeListMonastirComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Servicemateriaux.getMateriauxEquipement().subscribe((data: any) => {
      ...
      this.AgenciesCoffee = data.filter((item: any) => item.phone_number_commercial === "yes" && item.region === "monastir");

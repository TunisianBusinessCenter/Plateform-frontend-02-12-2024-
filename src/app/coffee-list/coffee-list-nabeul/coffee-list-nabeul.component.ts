...
@Component({
  selector: 'app-coffee-list-nabeul',
  templateUrl: './coffee-list-nabeul.component.html',
  styleUrls: ['./coffee-list-nabeul.component.css']
})
export class CoffeeListNabeulComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Servicemateriaux.getMateriauxEquipement().subscribe((data: any) => {
      ...
      this.AgenciesCoffee = data.filter((item: any) => item.phone_number_commercial === "yes" && item.region === "nabeul");
